"""
Report Trainer (fixed strong opponent) for Hockey environment.


Trains/evals exclusively against strong scripted opponent.
No opponent mixing/sampling for reproducible report baselines.
...
class ReportTrainer:
    """
    Report trainer: trains and evaluates against fixed strong opponent only.
    No random sampling or mixing for reproducible baselines.
    """
    ...
    def __init__(self, agent: BaseAgent, config: RLConfig):
        ...
        self.env = HockeyEnvWrapper(mode=config.mode, opponent='strong')
        ...
        print("ReportTrainer: Fixed strong opponent (training + eval)")
    ...
    def train(self) -> None:
        ...
        print(f"Final Evaluation (vs strong): {final_reward:.2f}")
        print("=" * 60)
    ...
    def evaluate(self, num_episodes: int = 5) -> float:
        """
        Evaluate vs strong opponent using only terminal win/loss rewards (+10/0/-10).
        """
        eval_rewards = []
        ...
        for ep in range(num_episodes):
            ...
            while not done:
                ...
                # Only win/loss for eval consistency
                if done:
                    winner = info.get('winner', 0)  # 1=win, -1=loss, 0=draw
    ...
    def _hparams(self) -> dict:
            base = {
                ...
                "trainer_type": "ReportTrainer",
                "opponent": "strong_fixed",
            }
            base.update({
