...
REWARD_CFG = dict(smooth_sigma=2.5,  roll_window=40, band_alpha=0.15, line_width=1.0)
...
CRITIC_YLIM = {
    "half_cheetah": (-500, 1000),
    "lunar_lander":  (-100, 200),
    "pendulum":      (-100, 200),
}
...
for env_folder, env_title in ENVIRONMENTS:
    env_dir = os.path.join(BASE_DIR, env_folder)
    if not os.path.isdir(env_dir):
        print(f"[warn] Environment folder not found: {env_dir}")
        continue
    ...
    color_map = draw_curves(
        ax_reward,
        data_dir = os.path.join(env_dir, "rewards"),
        agents   = ALL_AGENTS,
        ylabel   = "Avg. Episode Reward", title = f"{env_title} - Episode Rewards",
        **REWARD_CFG,
    )

    draw_curves(
        ax_alpha,
        data_dir  = os.path.join(env_dir, "alpha"),
        agents    = SAC_AGENTS,
        ylabel    = "Entropy Temp. $\\alpha$", title = f"{env_title} - SAC $\\alpha$",
        color_map = {k: color_map[k] for k in dict(SAC_AGENTS) if k in color_map},
        **ALPHA_CFG,
    )
    ax_alpha.set_ylim(bottom=0, top=ALPHA_YMAX.get(env_folder, 0.008))

    draw_curves(
        ax_actor,
        data_dir  = os.path.join(env_dir, "actor_loss"),
        agents    = LOSS_AGENTS,
        ylabel    = "Actor Loss", title = f"{env_title} - Actor",
    ...
    draw_curves(
        ax_critic,
        data_dir  = os.path.join(env_dir, "critic_loss"),
        agents    = LOSS_AGENTS,
        ylabel    = "Critic Loss", title = f"{env_title} - Critic",
        color_map = {k: color_map[k] for k in dict(LOSS_AGENTS) if k in color_map},
        **CRITIC_CFG,
    )
    if CRITIC_YLIM.get(env_folder) is not None:
        ax_critic.set_ylim(*CRITIC_YLIM[env_folder])
    ...
    out_path = os.path.join(BASE_DIR, f"overview_{env_folder}.pdf")
