...
\ac{TD3}~\cite{fujimoto2018:TD3} extends \ac{DDPG}~\cite{lillicrap2015:DDPG} by adressing two 
fundamental failure modes of determenistic actor-critic methods:
overestimation bias in the critic and instability in polciy updates.
The actor encodes a \emph{deterministic} policy $a = \pi_\phi(s)$ and is trained to maximize 
expected discounted return via the determenistic policy gradient theorem~\cite{silver2014:DPG}:
\begin{equation}
  \nabla_\phi J(\phi)
...
Taking the minimum introduces a conservative bias that counteracts the systematic overastimation 
...
\end{equation}
This encourages the critics to learn a smoother value landscape, making individual 
erroneous peaks less exploitable.The noise standard deviation $\sigma$ and clip range $c$ are hyperparameters 
that control the effective radius of this smoothing.

\medskip
\textbf{Delayed policy updates}

Even with conservative targets and smoothed backups, updating the actor at every critic step introduces 
high variance into the policy gradient: the critic has not yet fully integrated the latest target information 
before its gradient is used to move the policy.
...
substantially reducing variance of policy updates and preventing the actor from over-adapting 
to transient critic errors~\cite{fujimoto2018:TD3}.
...
clipped double Q-learning reduces overestimation bias, target policy smoothing 
regularizes the value function against sharp local peaks,
...
All modifications described below are applied on top of the \ac{TD3} and \ac{SAC} base implementations.
...
However, uniform sampling treats all transitions equally, ignoring that transitions with 
large \ac{TD} errors may provide stronger learning signal. 
...
The environment provides sparse rewards for winning and losing, with puck contact and direction signals disabled by default. 
