...
DQN~\cite{mnih2015:DQN} inapplicable, and on-policy algorithms such as
PPO~\cite{SchulmanEtAl2017:PPO} less attractive due to poor sample efficiency.
We therefore adopt two off-policy, continuous-control algorithms that share a common backbone
yet represent complementary design philosophies: TD3~\cite{fujimoto2018:TD3} optimizes a
deterministic policy with explicit bias-correction mechanisms, while SAC~\cite{haarnoja2018soft}
...
TD3~\cite{fujimoto2018:TD3} extends DDPG~\cite{lillicrap2015:DDPG} by adressing two fundamental failure modes of determenistic actor-critic methods:
...
% Improve, i dont like the DDPG-style (not scientific), also "this yields highly informative gradient signal..." to vague, not really scientif. total rewrite
While this yields highly informative gradient signal in continuous domains, 
DDPG-style methods are acutely sensitive to critic inaccuracies: whenever the critic overestimates the value of certain actions, 
the actor exploits these errors and converges to suboptimal or unstable behaviour~\cite{fujimoto2018:TD3}. 

TD3 adresses this with three interacting mechanisms.
...
% Improve, to complex sentence
Because the target $y$ is shared but gradients flow independently, the two critics diverge slightly, which is precisely what prevents correlated overestimation.
...
TD3 therfore updates the actor every $K$ critic steps % add here that K is most of the time 2
...
% I dont like this, bit to long, to long sentence, also not very scientific. We cant just say it ist a stable method...
Together, these three mechanisms form a coherent defence against the instabilities of deterministic actor-critic learning. Clipped double Q-learning bounds the magnitude of value overestimation, target policy smoothing prevents the actor from locking onto narrow value peaks, and delayed updates ensure the policy only improves once the critic has formed a reliable local picture of the value landscape. This combination produces a stable and data-efficient learning dynamic in continuous action spaces.
